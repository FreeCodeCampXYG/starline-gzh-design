...
**签名文案适配**：SKILL.md 的作者签名（“我是 {{作者名}}…”两段；生成时必须用本次请求或项目 `DEV_STATE.md` 的已确认发布身份替换）以正文段落（组件 5）形式放在本组件**之前**；本组件内部 `{{互动文案}}` 直接使用 SKILL.md 固定的第二段（“如果你觉得今天这篇有收获…三连，我们下篇见”），不要重复堆叠两句“三连”。
